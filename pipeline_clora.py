...
def erode(image, kernel_size=3, stride=1, padding=1):
    # Assuming image is a 4D tensor [Batch, Channels, Height, Width]
    # and binary where image values are 0 or 1
    # We invert the image, apply max pooling (dilation), and then invert the result
...
def dilate(image, kernel_size=3, stride=1, padding=1):
    # Assuming image is a 4D tensor [Batch, Channels, Height, Width]
    # and binary where image values are 0 or 1
    # We use max pooling to simulate dilation
...
class CloraPipeline(
    ...
    def attention_map_to_mask(
        ...
        for attention_map, mask_indice_list in zip(attention_maps, mask_indices):
            ...
            if mask_indice_list:
                for mask_indice in mask_indice_list:
                    ...
                    multi_token_map = torch.where(
                        attn_map > (attn_map.max() * mask_thresholding_alpha), 1.0, 0.0
                    )
                    multi_token_maps.append(multi_token_map)

                mask = torch.where(
                    torch.stack(multi_token_maps).sum(dim=0) >= 1.0, 1.0, 0.0
                )
            else:
            ...
            if mask_dilate:
                mask = dilate(mask, kernel_size=3, stride=1, padding=1)
            if mask_erode:
                mask = erode(mask, kernel_size=3, stride=1, padding=1)
            if mask_opening:
                mask = erode(mask, kernel_size=3, stride=1, padding=1)
                mask = dilate(mask, kernel_size=3, stride=1, padding=1)
            if mask_closing:
        ...
        masks[0] = torch.where(masks.sum(dim=0) == 0, 1, masks[0])

        return (
            F.interpolate(masks.unsqueeze(1), size, mode="nearest")
            .unsqueeze(1)
            .repeat(1, 1, 4, 1, 1)
        )
    ...
    def loss_fn(
        ...
        # Choose attention maps for important tokens
    ...
    def update_latents(
        ...
        grads = torch.autograd.grad(
            loss.requires_grad_(True), [latent], retain_graph=True
        )[0]
    ...
    def predict_noise_and_aggregate_attention(
        ...
        **kwargs,
        ...
        for lora_name, prompt_embeds in zip(lora_list, prompt_embeds_list):
            ...
            attention_maps = attention_store.aggregate_attention(["down", "mid", "up"])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.enable_lora()
        prompt_embeds_list = []
        for lora_name, prompt, negative_prompt in zip(
            lora_list, prompt_list, negative_prompt_list
        ):
            lora_names = []
            lora_weights = []
        ...
        adapters = self.get_list_adapters()
        for lora_name in lora_list:
            if lora_name != "" and (
                lora_name not in adapters["unet"]
                or lora_name not in adapters["text_encoder"]
            ):
                raise ValueError(
                    f"LoRA {lora_name} is not active. Please activate the LoRA before using it."
                )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    self.unet.zero_grad()
                    # 8. Predict noise and aggregate attention
                    (
                        noise_preds,
                        attention_maps_list,
                    ) = self.predict_noise_and_aggregate_attention(
                        lora_list,
                        prompt_embeds_list,
                        latent_model_input,
                        t,
                    ...
                    if latent_update and i < max_iter_to_alter:
                        ...
                        if i in iterative_steps:
                            for _ in range(iterative_steps_steps):
                                latent_model_input = torch.cat([latents] * 2)
                                latent_model_input = self.scheduler.scale_model_input(
                                    latent_model_input, t
                                )
                    ...
                    noise_preds = [noise_pred.detach() for noise_pred in noise_preds]
                    attention_maps_list = [
                        attention_map.detach() for attention_map in attention_maps_list
                    ]
                ...
                if i >= apply_mask_after:
                    masks = self.attention_map_to_mask(
                        ...
                        size=(noise_preds.shape[-2], noise_preds.shape[-1]),
                        mask_thresholding_alpha=mask_threshold_alpha,
                ...
                masks_to_save.append(masks[:, 0, 0])
                attention_maps_to_save.append(attention_maps_list)

                noise_pred_uncond = (noise_pred_uncond * masks).sum(dim=0)
                noise_pred_text = (noise_pred_text * masks).sum(dim=0)
                masks = masks.sum(dim=0)

                noise_pred_uncond = noise_pred_uncond / masks
                noise_pred_text = noise_pred_text / masks
        ...
        return image, attention_maps_to_save, masks_to_save
